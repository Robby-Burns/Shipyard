...
*   **Pydantic task_input Character Limit & Pass-by-Reference Specification**: Resolved a same-instant startup failure where generating a full multi-section engineering specification exceeded the Pydantic schema validation length constraint of 2000 characters on `AgentExecutionRequest.task_input`, terminating the workflow run before the Coordinator agent could start. Resolved by defining a dedicated, optional `specification_ref: uuid.UUID` field on the request schema to pass the database reference instead of raw text, updating the `BaseAgent.run` executor to lazily resolve specifications from the database with defensive validation checks, and routing this reference cleanly across Coordinator, Architect, and Builder agent steps.
*   **Terminated Run UI Retry and Warning Affordances**: Added a dedicated alert warning card on the SPA frontend when a workflow run transitions to a `failed` (Terminated) status, detailing the specific execution crash error messages (`project.error_message`) and exposing clear "Restart Build" controls (calling `/run?force=true`). Cleaned up the force-restart endpoint state machine transaction in the API handler to additionally wipe old manager approval parameters (`approved_by`, `approved_at`) for clean re-entry.
